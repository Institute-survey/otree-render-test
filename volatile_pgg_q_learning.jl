...
function print_help()
    println("""
Run expanded PGG simulations with subjective-net-profit Q-learning.

Options:
  --output-dir DIR          Output directory (default: expanded_pgg_outputs_subjective_netprofit_qlearning)
...
function parse_args(argv::Vector{String})
    output_dir = "expanded_pgg_outputs_subjective_netprofit_qlearning"
    groups = 100
    rounds = 100
    workers = nothing
    seed = 12345
...
@everywhere begin
    ...
    const RANDOM_DIRECTION_CHOICES = [-1, 0, 1]
    ...
    function subjective_net_profit(contribution::Float64, public_share::Float64, earned::Bool, cost_weight_increase::Float64)
        cost_multiplier = earned ? (1.0 + cost_weight_increase) : 1.0
    ...
    function simulate_group(condition::Condition, rounds::Int, seed::Int)
        ...
        q_values = zeros(Float64, GROUP_SIZE, ACTION_LEVELS)
        ...
        for t in 1:rounds
            ...
            subjective_u = Vector{Float64}(undef, GROUP_SIZE)
            for i in 1:GROUP_SIZE
                subjective_u[i] = subjective_net_profit(
                    current_contributions[i],
                    public_share_each,
                    condition.earned,
                    condition.cost_weight_increase,
                )
            end
            ...
            for i in 1:GROUP_SIZE
                best_idx = argmax(view(q_values, i, :))

                reference_amount = current_contributions[i]
                base_idx = nearest_index(next_grid, reference_amount)

                direction = if best_idx > current_indices[i]
                    1
                elseif best_idx < current_indices[i]
                    -1
                else
                    0
                end
                ...
                if condition.error_mode != "none" && rand(rng) < p_error
                    if condition.error_mode == "flip"
                        direction = direction != 0 ? -direction : rand(rng, DIRECTION_CHOICES)
                    elseif condition.error_mode == "random"
                        direction = rand(rng, RANDOM_DIRECTION_CHOICES)
                    else
                ...
                new_index = clip_index(base_idx + direction + gratitude_extra_steps)
    ...
    function write_outputs(output_dir::AbstractString, conditions::Vector{Condition}, results_by_condition::Dict{String, Vector{Vector{Float64}}}, rounds::Int)
        ...
        for cond in conditions
            cid = condition_id(cond)
            csv_path = joinpath(condition_dir, "$(cid).csv")
        ...
        sort!(manifest_rows, by = row -> (row.condition_type, row.condition_id))
        manifest_header = [
            "condition_type",
            "rho",
            "sigma_e",
        ...
        sort!(combined_rows, by = row -> (row.condition_type, row.condition_id, row.group_id))
